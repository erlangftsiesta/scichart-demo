...
export class ExampleDataProvider {
  ...
  public static getSinewave(
    amplitude: number,
    phase: number,
    pointCount: number,
    frequency: number = 10,
  ): IXyValues {
    return ExampleDataProvider.getDampedSinewave(
      0,
      amplitude,
      phase,
      0.0,
  ...
  public static getFourierSeriesZoomed(
    ...
    const xValues: number[] = fourierData.xValues.filter(
      (_, i) => i >= index0 && i < index1,
    );
    const yValues: number[] = fourierData.yValues.filter(
      (_, i) => i >= index0 && i < index1,
    );
    return { xValues, yValues };
  }

  public static getFourierSeries(
    amplitude: number,
    phaseShift: number,
    count: number = 5000,
  ): IXyValues {
  ...
  public static getExponentialCurve(
    power: number,
    pointCount: number,
  ): IXyValues {
    const xValues: number[] = [];
  ...
  public static getSpectrumData(
    ...
    const yValues = ExampleDataProvider.getSpectrum(
      points,
      arr.map(
        (_, i) => i / scaling + (i / scaling) * Math.random() * randomFactor,
      ),
      arr,
      shift,
    );
  ...
  static getRandomCandles = (
    ...
    const startTime =
      Math.floor(startDate.getTime() / 1000 / interval) * interval;
    let p: TPriceBar = {
      date: startTime,
      open: startPrice,
      high: startPrice,
...
export const fetchPopulationDataData =
  async (): Promise<TMappedPopulationData> =>
...
export const fetchGeoJson = (mapName: string) =>
  fetch(baseUrl + mapName + ".json").then((response) => response.json());
